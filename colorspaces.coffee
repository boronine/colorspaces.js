...
conv =
  from_CIEXYZ:
    ...
    to_CIELUV: (_X, _Y, _Z) ->
      ...
      [_L, _U, _V]
    to_CIELCHuv: (tuple...) ->
      luv = conv.from_CIEXYZ.to_CIELUV tuple...
      # Cylindrical transformation is identical for CIELAB and CIELUV
      conv.from_CIELAB.to_CIELCH luv...
    to_CIELCH: (tuple...) ->
  ...
  from_CIELCHuv:
    to_CIEXYZ: (tuple...) ->
      # Reverse cylindrical transformation is the same for CIELAB and CIELUV
      luv = conv.from_CIELCH.to_CIELAB tuple...
      conv.from_CIELUV.to_CIEXYZ luv...
  from_hex:
    to_sRGB: (hex) ->
      if hex.charAt(0) is "#"
